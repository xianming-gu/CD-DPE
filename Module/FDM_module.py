...
class adjoint_dictionary_model(nn.Module):
    ...
    def forward(self, y):
        ...
        for layer_idx in range(self.num_hidden_layers - 1, -1, -1):  # 2
            adjoint_syn_up = self.adjoint_syn_up_list[layer_idx]  # 下采样
            y, x = adjoint_syn_up(y)  # 1 96 120 120 , 1 128 60 60
            x_list.append(x)  # 1 64 240 240, 1 96 120 120
        y = self.adjoint_syn_bottleneck_conv(y)  # 1 128 60 60
        x_list.append(y)
        x_list.reverse()
        return x_list
...
class ista_unet(nn.Module):
    def  __init__(self, in_channel=1, out_channel=64, kernel_size=3, hidden_layer_width_list=[256, 128, 64],
                 n_classes=64, ista_num_steps=6, lasso_lambda_scalar=0.01):
        ...
        self.layer_in_x = nn.Conv2d(in_channel, n_classes, 3, 1, 1)
        self.layer_in_y = nn.Conv2d(in_channel, n_classes, 3, 1, 1)
        self.layer_in_z = nn.Conv2d(in_channel * 2, n_classes, 3, 1, 1)
        self.relu = nn.ReLU()
        self.cpmpress_z = nn.Conv2d(n_classes * 2, n_classes, 3, 1, 1)
        self.rec_x_unique = nn.Conv2d(n_classes, out_channel, 3, 1, 1)
        self.rec_x_common = nn.Conv2d(n_classes, out_channel, 3, 1, 1)
        self.rec_y_unique = nn.Conv2d(n_classes, out_channel, 3, 1, 1)
        self.rec_y_common = nn.Conv2d(n_classes, out_channel, 3, 1, 1)

        self.net_T = Offset_net(nf=64)

    def forward(self, x, y, z):
        # learned hyper-params
        ista_stepsize_iter_list_x = self.ista_stepsize_iter_list_x  # list:5*[1,]
        lasso_lambda_iter_list_x = self.lasso_lambda_iter_list_x  # list:15*[1,128,1,1]
        ista_stepsize_iter_list_y = self.ista_stepsize_iter_list_y  # list:5*[1,]
        lasso_lambda_iter_list_y = self.lasso_lambda_iter_list_y  # list:15*[1,128,1,1]
        ista_stepsize_iter_list_z = self.ista_stepsize_iter_list_z  # list:5*[1,]
        lasso_lambda_iter_list_z = self.lasso_lambda_iter_list_z  # list:15*[1,128,1,1]

        ## initialize MS feature: x_list, y_list, z_list
        x_in = self.layer_in_x(x)
        y_in = self.layer_in_y(y)
        z_in = self.layer_in_z(z)  # 1 64 240 240
        ...
        for idx in range(1, self.ista_num_steps):  # 5
            err_x = self.encoder_dictionary_x(x_list) + self.encoder_dictionary_zx(z_list)  # 1 64 240 240
            ### alignment z->y  ########## ori
            cur_y_fea = self.encoder_dictionary_y(y_list)  # 1 64 240 240
            cur_z_fea = self.encoder_dictionary_zy(z_list)  # 1 64 240 240
            img_offset_zy, attention_map = self.net_T(cur_z_fea, cur_y_fea)  # 1 2 240 240 , 1 1 240 240
            cur_z_fea_aligned = warp(cur_z_fea, img_offset_zy)
            err_y = cur_y_fea + attention_map * cur_z_fea_aligned  # 1 64 240 240
            ###### No alignmnet ##### no need align
